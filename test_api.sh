...
RESPONSE1=$(curl -s -X POST https://loopai-28k9.onrender.com/api/v1/ingest \
...
RESPONSE2=$(curl -s -X POST https://loopai-28k9.onrender.com/api/v1/ingest \
  -H "Content-Type: application/json" \
...
curl -s https://loopai-28k9.onrender.com/api/v1/status/$INGESTION_ID_1 | json_pp

echo -e "\nChecking status of second request (HIGH priority)..."
curl -s https://loopai-28k9.onrender.com/api/v1/status/$INGESTION_ID_2 | json_pp
